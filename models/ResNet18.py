# resnet18_emotion_detection_with_plots.py
import cv2
import torch
...
class ResNet18EmotionDetectorWithPlots:
    def __init__(self, model_path=r"C:\Users\LENOVO\OneDrive\Desktop\Debasish\FER Project\resnet18_best.pth"):
        # Device setup
    ...
    def run_real_time(self):
        ...
        print("\n🎥 Real-Time Facial Expression Recognition - ResNet18!")
        print("📋 Controls:")
        ...
        while True:
            ...
            # Info overlay
            cv2.putText(frame, "Debasish's FER Project - ResNet18", 
            ...
            cv2.imshow('Debasish FER Project - ResNet18 Real-Time Detection', frame)
            
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                break
            elif key == ord('s'):
                filename = f'resnet18_capture_{int(time.time())}.jpg'
        ...
        print("🏁 ResNet18 real-time detection ended with plots generated!")
    ...
    def show_current_plots(self):
        ...
        print(f"\n📊 ResNet18 Current Statistics (Total: {total} detections):")
    ...
    def generate_final_plots(self):
        ...
        fig.suptitle('Debasish\'s Real-Time Facial Expression Recognition - ResNet18 Results', 
        ...
        if len(self.confidence_history) > 0:
            ax3.plot(list(self.confidence_history), color='blue', linewidth=1.5)
        ...
        if len(self.fps_history) > 0:
            ax4.plot(list(self.fps_history), color='red', linewidth=2, marker='s', markersize=3)
            ...
            ax4.axhline(y=avg_fps, color='blue', linestyle='--', alpha=0.8, 
        ...
        if len(self.confidence_history) > 0:
            n, bins, patches = ax5.hist(list(self.confidence_history), bins=15, 
                                       alpha=0.7, color='darkblue', edgecolor='black', linewidth=0.5)
        ...
        # Save plot
        filename = f'resnet18_results_{int(time.time())}.png'
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"📊 ResNet18 plots saved as: {filename}")
        ...
        if self.session_data:
            ...
            csv_filename = f'resnet18_session_data_{int(time.time())}.csv'
            df.to_csv(csv_filename, index=False)
            print(f"💾 ResNet18 session data saved as: {csv_filename}")


# Run the ResNet18 application
if __name__ == "__main__":
    detector = ResNet18EmotionDetectorWithPlots()
