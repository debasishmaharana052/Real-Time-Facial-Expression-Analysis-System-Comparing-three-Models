# mobilenetv2_emotion_detection_with_plots.py
import cv2
import torch
...
class MobileNetV2EmotionDetectorWithPlots:
    def __init__(self, model_path=r"C:\Users\LENOVO\OneDrive\Desktop\Debasish\FER Project\mobilenet_v2_best.pth"):
        # Device setup
        ...
        print(f"Loading MobileNetV2 model from: {model_path}")
    ...
    def run_real_time(self):
        ...
        print("\n🎥 Real-Time Facial Expression Recognition - MobileNetV2!")
        print("📋 Controls:")
        ...
        while True:
            ...
            # Info overlay
            cv2.putText(frame, "Debasish's FER Project - MobileNetV2", 
            ...
            cv2.imshow('Debasish FER Project - MobileNetV2 Real-Time Detection', frame)
            
            key = cv2.waitKey(1) & 0xFF
            if key == ord('q'):
                break
            elif key == ord('s'):
                filename = f'mobilenetv2_capture_{int(time.time())}.jpg'
        ...
        print("🏁 MobileNetV2 real-time detection ended with plots generated!")
    ...
    def show_current_plots(self):
        ...
        print(f"\n📊 MobileNetV2 Current Statistics (Total: {total} detections):")
    ...
    def generate_final_plots(self):
        ...
        fig.suptitle('Debasish\'s Real-Time Facial Expression Recognition - MobileNetV2 Results', 
        ...
        # Save plot
        filename = f'mobilenetv2_results_{int(time.time())}.png'
        plt.savefig(filename, dpi=300, bbox_inches='tight', facecolor='white')
        print(f"📊 MobileNetV2 plots saved as: {filename}")
        ...
        if self.session_data:
            ...
            csv_filename = f'mobilenetv2_session_data_{int(time.time())}.csv'
            df.to_csv(csv_filename, index=False)
            print(f"💾 MobileNetV2 session data saved as: {csv_filename}")


# Run the MobileNetV2 application
if __name__ == "__main__":
    detector = MobileNetV2EmotionDetectorWithPlots()
